...
uint8_t *USBD_WinUSBOSStrDescriptor(struct _USBD_HandleTypeDef *pdev, uint8_t index,  uint16_t *length);
...
__ALIGN_BEGIN static uint8_t USBD_WinUSB_CfgHSDesc[WINUSB_CONFIG_DESC_SIZE] __ALIGN_END =
{
...
//  0x00,   /* bInterfaceClass: vendor */
  ...
  USBD_WinUSB_OUT_EP,                        /* bEndpointAddress */
  0x02,                              /* bmAttributes: Bulk */
  LOBYTE(USBD_WinUSB_DATA_HS_MAX_PACKET_SIZE),  /* wMaxPacketSize: */
  HIBYTE(USBD_WinUSB_DATA_HS_MAX_PACKET_SIZE),
  0x00,                              /* bInterval: ignore for Bulk transfer */
  
  /*Endpoint IN Descriptor*/
  0x07,   /* bLength: Endpoint Descriptor size */
  USB_DESC_TYPE_ENDPOINT,      /* bDescriptorType: Endpoint */
  USBD_WinUSB_IN_EP,                         /* bEndpointAddress */
  0x02,                              /* bmAttributes: Bulk */
  LOBYTE(USBD_WinUSB_DATA_HS_MAX_PACKET_SIZE),  /* wMaxPacketSize: */
  HIBYTE(USBD_WinUSB_DATA_HS_MAX_PACKET_SIZE),
  0x00                               /* bInterval: ignore for Bulk transfer */
...
//  0x00,   /* bInterfaceClass: vendor */
  ...
  USBD_WinUSB_OUT_EP,                        /* bEndpointAddress */
  0x02,                              /* bmAttributes: Bulk */
  LOBYTE(USBD_WinUSB_DATA_FS_MAX_PACKET_SIZE),  /* wMaxPacketSize: */
  HIBYTE(USBD_WinUSB_DATA_FS_MAX_PACKET_SIZE),
  0x00,                              /* bInterval: ignore for Bulk transfer */
...
__ALIGN_BEGIN static uint8_t USBD_WinUSB_OtherSpeedCfgDesc[WINUSB_CONFIG_DESC_SIZE] __ALIGN_END =
{
  /*Configuration Descriptor*/
  0x09,   /* bLength: Configuration Descriptor size */
  USB_DESC_TYPE_CONFIGURATION,      /* bDescriptorType: Configuration */
...
static uint8_t USBD_WinUSB_Init(USBD_HandleTypeDef *pdev, uint8_t cfgidx)
{
  ...
  hcdc = USBD_malloc(sizeof(USBD_WinUSB_HandleTypeDef));
  ...
  {
    /* Open EP IN */
    (void)USBD_LL_OpenEP(pdev, USBD_WinUSB_IN_EP, USBD_EP_TYPE_BULK,
                         USBD_WinUSB_DATA_HS_IN_PACKET_SIZE);

    pdev->ep_in[USBD_WinUSB_IN_EP & 0xFU].is_used = 1U;

    /* Open EP OUT */
    (void)USBD_LL_OpenEP(pdev, USBD_WinUSB_OUT_EP, USBD_EP_TYPE_BULK,
                         USBD_WinUSB_DATA_HS_OUT_PACKET_SIZE);

    pdev->ep_out[USBD_WinUSB_OUT_EP & 0xFU].is_used = 1U;
...
//    pdev->ep_in[CDC_CMD_EP & 0xFU].bInterval = CDC_HS_BINTERVAL;
  ...
  else
  {
    /* Open EP IN */
    (void)USBD_LL_OpenEP(pdev, USBD_WinUSB_IN_EP, USBD_EP_TYPE_BULK,
                         USBD_WinUSB_DATA_FS_IN_PACKET_SIZE);

    pdev->ep_in[USBD_WinUSB_IN_EP & 0xFU].is_used = 1U;

    /* Open EP OUT */
    (void)USBD_LL_OpenEP(pdev, USBD_WinUSB_OUT_EP, USBD_EP_TYPE_BULK,
                         USBD_WinUSB_DATA_FS_OUT_PACKET_SIZE);
...
//  pdev->ep_in[CDC_CMD_EP & 0xFU].is_used = 1U;
  ...
  if (pdev->dev_speed == USBD_SPEED_HIGH)
  {
    ...
    (void)USBD_LL_PrepareReceive(pdev, USBD_WinUSB_OUT_EP, hcdc->RxBuffer,
                                 USBD_WinUSB_DATA_HS_OUT_PACKET_SIZE);
  }
  else
  {
...
static uint8_t USBD_WinUSB_DeInit(USBD_HandleTypeDef *pdev, uint8_t cfgidx)
{
  ...
  /* Close EP IN */
  (void)USBD_LL_CloseEP(pdev, USBD_WinUSB_IN_EP);
  pdev->ep_in[USBD_WinUSB_IN_EP & 0xFU].is_used = 0U;

  /* Close EP OUT */
  (void)USBD_LL_CloseEP(pdev, USBD_WinUSB_OUT_EP);
  pdev->ep_out[USBD_WinUSB_OUT_EP & 0xFU].is_used = 0U;
...
static uint8_t USBD_WinUSB_DataIn(USBD_HandleTypeDef *pdev, uint8_t epnum)
{
  ...
  else
  {
    ...
    if (((USBD_WinUSB_ItfTypeDef *)pdev->pUserData)->TransmitCplt != NULL)
    {
      ((USBD_WinUSB_ItfTypeDef *)pdev->pUserData)->TransmitCplt(hcdc->TxBuffer, &hcdc->TxLength, epnum);
    }
  }
...
static uint8_t *USBD_WinUSB_GetHSCfgDesc(uint16_t *length)
{
  *length = (uint16_t)sizeof(USBD_WinUSB_CfgHSDesc);
...
static uint8_t *USBD_WinUSB_GetOtherSpeedCfgDesc(uint16_t *length)
{
  *length = (uint16_t)sizeof(USBD_WinUSB_OtherSpeedCfgDesc);
...
uint8_t *USBD_WinUSB_GetDeviceQualifierDescriptor(uint16_t *length)
{
  *length = (uint16_t)sizeof(USBD_WinUSB_DeviceQualifierDesc);
...
uint8_t USBD_WinUSB_TransmitPacket(USBD_HandleTypeDef *pdev)
{
  ...
  if (hcdc->TxState == 0U)
  {
    ...
    /* Update the packet total length */
    pdev->ep_in[USBD_WinUSB_IN_EP & 0xFU].total_length = hcdc->TxLength;

    /* Transmit next packet */
    (void)USBD_LL_Transmit(pdev, USBD_WinUSB_IN_EP, hcdc->TxBuffer, hcdc->TxLength);
